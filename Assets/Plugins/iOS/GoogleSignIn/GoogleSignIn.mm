...
GIDSignInResult *SignResult = nil;
...
- (void)signIn:(GIDSignIn *)signIn
  ...
  UnityPause(true);
    ViewController = viewController; NSLog(@"presentViewController");
  [UnityGetGLViewController() presentViewController:viewController
                                           animated:YES
                                         completion:nil];
}

GIDConfiguration *signInConfig = NULL;
NSString *loginHint = NULL;
NSArray<NSString *> *scopes = NULL;
...
bool GoogleSignIn_Configure(void *unused, bool useGameSignIn,
    ...
    NSLog(@"GoogleSignIn_Configure");
  if (webClientId) {
      signInConfig = [[GIDConfiguration alloc] initWithClientID:[NSString stringWithUTF8String:webClientId]];
  ...
  if (accountName) {
    loginHint = [NSString stringWithUTF8String:accountName];
  }
...
void *GoogleSignIn_SignIn() {
  ...
  if (!result) {
      ...
      [GIDSignIn.sharedInstance signInWithPresentingViewController:UnityGetGLViewController()  hint:loginHint additionalScopes:scopes completion:^(GIDSignInResult * _Nullable user, NSError * _Nullable error) {
          if (error== nil) {
              SignResult = user;
            if (currentResult_) {
...
size_t GoogleSignIn_GetServerAuthCode(GIDGoogleUser *guser, char *buf,
  ...
  if(SignResult)
  {
      NSString *val = SignResult.serverAuthCode;
...
size_t GoogleSignIn_GetIdToken(GIDGoogleUser *guser, char *buf, size_t len) {
  GIDToken *token22 = [guser idToken];
  return CopyNSString(token22.tokenString, buf, len);
}
